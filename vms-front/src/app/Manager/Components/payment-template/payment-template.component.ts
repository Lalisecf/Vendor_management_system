...
@Component({
  ...
  styleUrls: ['./payment-template.component.css']
})
export class PaymentTemplateComponent implements OnInit, AfterViewInit {
  ...
  getPayments(paymentId: any) {
    this.makerService.get_payment_by_itsId(paymentId, 0).subscribe(
      (data: any) => {
        this.pymt = data[0];
        this.getContractbyId(data[0].contract_id)
      },
      (error: any) => {
  ...
  getManager(name: any) {
    this.adminService.get_name_of_user(name).subscribe(
      (data: any) => {
        this.manager = data;
      },
      (error: any) => {
  ...
  ngOnInit(): void {
    this.getPayments(this.payment_id)
