...
@Component({
  selector: 'app-requested-addendum-payments',
  templateUrl: './requested-addendum-payments.component.html',
  styleUrls: ['./requested-addendum-payments.component.css'],
  providers: [
    DatePipe
  ]
})
export class RequestedAddendumPaymentsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.managerService.getRequestedAddendumPaymentDetail().subscribe(
          async (resp: any) => {
            if (resp != null) {
              callback({
        ...
        },
        {
          ...
          render: (data: any, type: any, full: any) => {
            if (full.status === '2' || full.status === '4' || full.status === '6'|| full.status === '5') {
              return `<i class="mdi mdi-eye text-primary ml-2" style="cursor: pointer;" addedum-payment-detail="${full.id}">Detail</i>`;
            }
            return ''; 
          }
        }
