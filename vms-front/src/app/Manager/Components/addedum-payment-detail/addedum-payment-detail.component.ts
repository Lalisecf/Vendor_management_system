...
@Component({
  selector: 'app-addedum-payment-detail',
  templateUrl: './addedum-payment-detail.component.html',
  styleUrls: ['./addedum-payment-detail.component.css']
})
export class AddedumPaymentDetailComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen('document', 'click', (event) => {
      ...
      if (event.target.hasAttribute('addedum-payment-detail')) {
        const paymentDetailId = event.target.getAttribute('addedum-payment-detail');
        if (userRoles.includes('Director')) {
          this.router.navigateByUrl('Director/addedum-payment-detail/' + paymentDetailId);
        }else if (userRoles.includes('Finance')) {
            this.router.navigateByUrl('Finance/addedum-payment-detail/' + paymentDetailId);
        }else if (userRoles.includes('Chief')) {
            this.router.navigateByUrl('Chief/addedum-payment-detail/' + paymentDetailId);
        } else {
          this.router.navigateByUrl('Manager/addedum-payment-detail/' + paymentDetailId);
        }
      } 
  ...
  getPaymnet() {
    this.managerService.getAddendumPaymentById(this.payment_id).subscribe(
      async (data: any) => {
        ...
        if (data!= null){
          ...
          this.paymentDetail.addendum_id = data.addendum_id
    ...
    Swal.fire({
     ...
     // title: 'Approve contract with title : ' + this.paymentDetail.payment_description,
      ...
      if (result.isConfirmed) {
        this.managerService.approveAddendumPayment(this.payment_id).subscribe(
          async (data) => {
            if (data == true) {
              Swal.fire({
                icon: 'success',
      ...
      const userRoles = roles.map((role) => role.name);
      if (userRoles.includes('Director')) {
        this.router.navigateByUrl('/Director/View-Requested-Addendum-payment');
      }else if (userRoles.includes('Finance')) {
        this.router.navigateByUrl('/Finance/View-Requested-Addendum-payment');
      }else if (userRoles.includes('Chief')) {
        this.router.navigateByUrl('/Chief/View-Requested-Addendum-payment');
      }else{
        this.router.navigateByUrl('/Manager/View-Requested-Addendum-payment');
      }
  ...
  RejectPaymentRequest(id: any, remark: Remark, reason: any) {
    ...
    this.managerService.RejectAddendumPaymentRequest(id, remark, reason).subscribe(
      (data) => {
        if (data) {
