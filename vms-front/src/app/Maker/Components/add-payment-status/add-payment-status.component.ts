...
export class AddPaymentStatusComponent implements OnInit {
  ...
  @ViewChild('SuccessSwalRegisterPaymentStatus')
  public readonly SuccessSwalRegisterPaymentStatus!: SwalComponent;
  @ViewChild('SuccessSwalUpdatePaymentStatus')
  public readonly SuccessSwalUpdatePaymentStatus!: SwalComponent;
  dropdownList = [];
  selectedItems = [];
  dropdownSettings = {};
  payment_ststus_id: any;
  user_role: any;
  ...
  registerPaymentStatusPayload!: Product;
  ...
  constructor(
    ...
    this.payment_ststus_id = this.activatedRoute.snapshot.paramMap.get('id');
    // this.user_role = this.activatedRoute.snapshot.paramMap.get('role');
    // this.checkEmailExistPayload = {
    //   email: '',
    // };
    this.registerPaymentStatusPayload = {
      id: null,
      name: '',
      description: '',
      status: '',
  ...
  ngOnInit(): void {
    if (this.payment_ststus_id != null) {
  ...
  rrr: any;
  getPaymentStatus() {
    this.makerService.getPaymentStatus(this.payment_ststus_id).subscribe(
      (data: any) => {
        this.user = data;
  ...
  onSubmit() {
    if (
      ...
      this.registerPaymentStatusPayload = this.registerForm.value;
      if (this.payment_ststus_id != null) this.registerPaymentStatusPayload.id = this.payment_ststus_id;
      console.log('==================dsdfdsf')
      this.registerPaymentStatus(this.registerPaymentStatusPayload);
    } else this.submitted = true;
  }

  registerPaymentStatus(registerActorPayload: Product) {
    console.log(
      'Payload: ' + JSON.stringify(this.registerPaymentStatusPayload, null, 2)
    );
    this.makerService.registerPaymentStatus(registerActorPayload).subscribe(
      async (data) => {
        if (data == true) {
          this.SuccessSwalRegisterPaymentStatus.fire();

          if (this.payment_ststus_id != null) {
            this.SuccessSwalUpdatePaymentStatus.fire();
            this.router.navigateByUrl('/maker/view-payment-status');
