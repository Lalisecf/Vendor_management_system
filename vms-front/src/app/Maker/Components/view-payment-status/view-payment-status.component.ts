...
@Component({
  selector: 'app-view-payment-status',
  templateUrl: './view-payment-status.component.html',
  styleUrls: ['./view-payment-status.component.css']
})
export class ViewPaymentStatusComponent implements OnInit, AfterViewInit, OnDestroy {
  dtOptions: any;
  // min!: number;
  // max!: number;
  //Swal Config
  ...
  second_column_title = 'Payment Status';
          ...
          render: function (data: any, type: any, full: any) {
            ...
            if (full.status == 1)
              return (
                ...
                'deactivate-payment_status-name="' +
                full.name +
                '" deactivate-payment_status="' +
                full.id +
            ...
            return (
              ...
              'activate-payment_status-name="' +
              full.name +
              '" activate-payment_status="' +
              full.id +
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            document
              ...
              ?.classList.remove('dt-button');
            return (
              '<i class="mdi mdi-18px mdi-pencil text-secondary" style="cursor: pointer;" edit-payment_status="' +
              full.id +
              '"></i>' +
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('edit-payment_status')) {
        this.router.navigateByUrl(
          'maker/view-payment-status/edit/' + event.target.getAttribute('edit-payment_status')
        );
      } else if (event.target.hasAttribute('delete-payment_status')) {
        var selected_contract_type_name = event.target.getAttribute('payment_status-name');
        Swal.fire({
          title: 'Delete Payment Status: ' + selected_contract_type_name,
          text:
            'Are you sure? you are about to delete ' +
            selected_contract_type_name +
            ' from the system.',
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            ...
            this.makerService
              .deleteContractPaymentStatus(event.target.getAttribute('delete-payment_status'))
              .subscribe(
                (data: any) => {
                    ...
                    (dtInstance: DataTables.Api) => {
                      dtInstance.ajax.reload((data) => {
                        ...
                        Swal.fire({
                          icon: 'success',
                          title: 'Success',
                          text: 'payment status deleted successfully!',
                        });
                      }, false);
                    }
                  );
                      ...
                      this.utilService.refreshToken().subscribe(
                        (data) => {
                          if (data === true) {
                            ...
                            //================================================================================
                            this.makerService
                              .deleteContractPaymentStatus(
                                event.target.getAttribute('delete-payment_status')
                              )
                              .subscribe(
                                (data: any) => {
                                  this.datatableElement.dtInstance.then(
            ...
            console.log(
              'delete-payment_status: ' + event.target.getAttribute('delete-payment_status')
            );
          }
        });
      } else if (event.target.hasAttribute('activate-payment_status')) {
        var contract_type_name = event.target.getAttribute('activate-payment_status-name');
        var contract_type_id = event.target.getAttribute('activate-payment_status');
        Swal.fire({
          title: 'Activate Payment Status: ' + contract_type_name,
          text:
            'Are you sure? you are about to activate ' +
            contract_type_name +
            '.This means the payment status will be accessed ',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.activatePaymentStatus(contract_type_id).subscribe(
              (data) => {
                if (data) {
                  Swal.fire({
                    icon: 'success',
      ...
      } else if (event.target.hasAttribute('deactivate-payment_status')) {
        var contract_type_name = event.target.getAttribute('deactivate-payment_status-name');
        var contract_type_id = event.target.getAttribute('deactivate-payment_status');
        Swal.fire({
          title: 'Deactivate Payment Status: ' + contract_type_name,
          text:
            'Are you sure? you are about to deactivate ' +
            contract_type_name +
            '. This means the payment status will be prevented from accessed.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#d33',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.deactivatePaymentStatus(contract_type_id).subscribe(
              (data) => {
                if (data == true) {
                  Swal.fire({
                    icon: 'success',
