...
@Component({
  selector: 'app-view-contract-type',
  templateUrl: './view-contract-type.component.html',
  styleUrls: ['./view-contract-type.component.css']
})
export class ViewContractTypeComponent implements OnInit, AfterViewInit, OnDestroy {
  dtOptions: any;
  // min!: number;
  // max!: number;
  //Swal Config
  ...
  second_column_title = 'Contract Type';
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            document
              ...
              ?.classList.remove('dt-button');
            return (
              '<i class="mdi mdi-18px mdi-pencil text-secondary" style="cursor: pointer;" edit-contract-type="' +
              full.id +
              '"></i>' +
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('edit-contract-type')) {
        this.router.navigateByUrl(
          'maker/view-contract-type/edit/' + event.target.getAttribute('edit-contract-type')
        );
      } else if (event.target.hasAttribute('delete-contract-type')) {
        var selected_contract_type_name = event.target.getAttribute('contract-type-name');
        Swal.fire({
          title: 'Delete Contract Type: ' + selected_contract_type_name,
          text:
          ...
          if (result.isConfirmed) {
            ...
            this.makerService
              .deleteContractType(event.target.getAttribute('delete-contract-type'))
              .subscribe(
                (data: any) => {
                      ...
                      this.utilService.refreshToken().subscribe(
                        (data) => {
                          if (data === true) {
                            ...
                            this.makerService
                              .deleteContractType(
                                event.target.getAttribute('delete-contract-type')
                              )
                              .subscribe(
                                (data: any) => {
                                  this.datatableElement.dtInstance.then(
            ...
            console.log(
              'delete-contract-type: ' + event.target.getAttribute('delete-contract-type')
            );
          }
        });
      } else if (event.target.hasAttribute('activate-contract-type')) {
        var contract_type_name = event.target.getAttribute('activate-contract-type-name');
        var contract_type_id = event.target.getAttribute('activate-contract-type');
        Swal.fire({
          title: 'Activate Contract Type: ' + contract_type_name,
          text:
            'Are you sure? you are about to activate ' +
            contract_type_name +
            '.This means the contract-type will be accessed ',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.activateContractType(contract_type_id).subscribe(
              (data) => {
                if (data) {
                  Swal.fire({
                    icon: 'success',
      ...
      } else if (event.target.hasAttribute('deactivate-contract-type')) {
        var contract_type_name = event.target.getAttribute('deactivate-contract-type-name');
        var contract_type_id = event.target.getAttribute('deactivate-contract-type');
        Swal.fire({
          title: 'Deactivate contract-type: ' + contract_type_name,
          text:
            'Are you sure? you are about to deactivate ' +
            contract_type_name +
            '. This means the contract-type will be prevented from accessed.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#d33',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.deactivateContractType(contract_type_id).subscribe(
              (data) => {
                if (data == true) {
                  Swal.fire({
                    icon: 'success',
