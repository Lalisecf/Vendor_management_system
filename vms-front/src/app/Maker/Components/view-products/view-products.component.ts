...
export class ViewProductsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  second_column_title = 'Category Name';
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            document
              ...
              ?.classList.remove('dt-button');
            return (
              '<i class="mdi mdi-18px mdi-pencil text-secondary" style="cursor: pointer;" edit-product="' +
              full.id +
              '"></i>' +
  ...
  ngAfterViewInit(): void {
    ...
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('edit-product')) {
        this.router.navigateByUrl(
          'maker/view-products/edit/' + event.target.getAttribute('edit-product')
        );
      } else if (event.target.hasAttribute('delete-product')) {
        var selected_product_name = event.target.getAttribute('product-name');
        Swal.fire({
          title: 'Delete Product: ' + selected_product_name,
          text:
          ...
          if (result.isConfirmed) {
            ...
            this.makerService
              .deleteProduct(event.target.getAttribute('delete-product'))
              .subscribe(
                (data: any) => {
                      ...
                      this.utilService.refreshToken().subscribe(
                        (data) => {
                          if (data === true) {
                            ...
                            this.makerService
                              .deleteProduct(
                                event.target.getAttribute('delete-product')
                              )
                              .subscribe(
                                (data: any) => {
                                  this.datatableElement.dtInstance.then(
            ...
            console.log(
              'delete-product: ' + event.target.getAttribute('delete-product')
            );
          }
        });
      } else if (event.target.hasAttribute('activate-product')) {
        var product_name = event.target.getAttribute('activate-product-name');
        var product_id = event.target.getAttribute('activate-product');
        Swal.fire({
          title: 'Activate Product: ' + product_name,
          text:
            'Are you sure? you are about to activate ' +
            product_name +
            '.This means the product will be accessed ',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.activateProduct(product_id).subscribe(
              (data) => {
                if (data) {
                  Swal.fire({
                    icon: 'success',
      ...
      } else if (event.target.hasAttribute('deactivate-product')) {
        var product_name = event.target.getAttribute('deactivate-product-name');
        var product_id = event.target.getAttribute('deactivate-product');
        Swal.fire({
          title: 'Deactivate product: ' + product_name,
          text:
            'Are you sure? you are about to deactivate ' +
            product_name +
            '. This means the product will be prevented from accessed.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#d33',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.makerService.deactivateProduct(product_id).subscribe(
              (data) => {
                if (data == true) {
                  Swal.fire({
                    icon: 'success',
