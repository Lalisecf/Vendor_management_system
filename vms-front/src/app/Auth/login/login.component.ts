...
export class LoginComponent implements OnInit, AfterViewChecked {
  ...
  onSubmit() {
    ...
    if (this.loginForm.valid) {
      this.login_payload.email = this.loginForm.get('email')?.value + '@aabank.com';
      ...
      this.authService.checkEmailExist(this.login_payload.email).subscribe(
        (data: Boolean) => {
          if (data == true) {
            this.authService.login_other_device_browser(this.login_payload.email).subscribe(
              (data: Boolean) => {
                if (data == false) {
                  this.authService.login(this.login_payload).subscribe(
                    ...
                    (error) => {
                      if (
                        ...
                        this.handleErrorMessage(error.error.text);
                      } else if (error.error.text === 'custom-bad-credentials') {
                        this.displayBadCredentialMessage();
                      } else if (
                        error.error.text != undefined &&
                        error.error.text.toString().includes('login-trial-exceeded')
                      ) {
                        this.displayLoginTrialExceededMessage();
                      } else if (
                        error.error.text != undefined &&
                        error.error.text.toString().includes('user_disabled')
                      ) {
                        this.displayUserDisabledMessage();
                      } else {
                  ...
                  Swal.fire({
                    title: '<p style="color:gold">Already logged in on another machine or browser!</p>',
  ...
  private handleErrorMessage(errors: string) {
    this.login_trial_exceeded = false;
    this.user_disabled = false;

    document.getElementById('bad-credential-message')?.classList.add('d-block');
    document.getElementById('bad-credential-message')?.classList.remove('d-none');
    document.getElementById('server-error-message')?.classList.add('d-none');
    document.getElementById('server-error-message')?.classList.remove('d-block');
    ...
    if (Number(message[0]) <= Number(message[2])) {
        if (Number(message[2]) - Number(message[0]) === 0) {
            ...
            console.log('Time difference: ' + this.timeDiff(new Date(message[6]), a));
            this.localStorageService.store('login_trials', this.login_payload.email);
            ...
            console.log('Message 0: ' + Number(message[0]));
            this.localStorageService.store('pt', Number(message[2]) - Number(message[0]));
        }
    } else if (Number(Number(message[0]) - Number(message[2])) <= Number(message[3])) {
        if (Number(Number(message[0]) - Number(message[2])) - Number(message[3]) === 0) {
            this.localStorageService.clear('pt');
  ...
  private displayBadCredentialMessage() {
    this.login_trial_exceeded = false;
    this.user_disabled = false;
    this.localStorageService.clear('pt');
    document.getElementById('bad-credential-message')?.classList.add('d-block');
  ...
  private displayLoginTrialExceededMessage() {
    this.user_disabled = false;
    this.localStorageService.clear('pt');
    this.login_trial_exceeded = true;
    document.getElementById('bad-credential-message')?.classList.add('d-block');
  ...
  private displayUserDisabledMessage() {
    this.login_trial_exceeded = false;
    this.localStorageService.clear('pt');
  ...
  private displayServerError() {
    this.login_trial_exceeded = false;
    this.user_disabled = false;
    document.getElementById('bad-credential-message')?.classList.remove('d-block');
    document.getElementById('bad-credential-message')?.classList.add('d-none');
    document.getElementById('server-error-message')?.classList.add('d-block');
    document.getElementById('server-error-message')?.classList.remove('d-none');
  }
