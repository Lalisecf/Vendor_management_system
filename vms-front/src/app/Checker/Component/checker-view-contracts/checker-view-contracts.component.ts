...
@Component({
  selector: 'app-checker-view-contracts',
  templateUrl: './checker-view-contracts.component.html',
  styleUrls: ['./checker-view-contracts.component.css'],
  providers: [
    DatePipe
  ]
})
export class CheckerViewContractsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  end_date!: any
  total_amount!: any
  extend_renew: boolean = false;
  pre_end_date: any;
  pre_total_amount: any;
  pre_contract_document_document: any;
  renew_contract_document_document: any;
  save_as!: any;
  status!: any;
  payment_id!: any;
  IdPayment!: any;
  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      columns: [
        ...
        //       return (
        //         '<i class="mdi mdi-18px mdi-download text-secondary" style="cursor: pointer;" download-sla="' +
        //         full.id +
        //         '"></i>Download SLA document'
        //       );
        //     else
        //       return '<span class=" ">No file uploaded</span>';
        //   },
        ...
        //         '" deactivate-contract="' +
        //         full.id +
        //         '" disabled><i class="mdi mdi-window-close mr-1"></i>Hold</button>'
        //       );
        ...
        //         '" activate-vendor="' +
        //         full.id +
        //         '" disabled><i class="mdi mdi-check"></i>Activate</button>'
        //       );
          ...
          render: function (data: any, type: any, full: any) {
            ...
            if (full.request_type == 'Hold')
              return (
                ...
                '"hold-contract="' +
                full.id +
                '"><i class="mdi mdi-window-close mr-1"></i>Hold Request</button>'
              );

            else if (full.request_type == 'Unhold')
              return (
                '<button  class="btn btn-outline-warning btn-sm"  ' +
                'unhold-contract-name="' +
                full.name +
                '" unhold-contract="' +
                full.id +
                '" ><i class="mdi mdi-window-close mr-1"></i>Unhold Request</button>'
              );

            else if (full.request_type == 'Extend')
              return (
                '<button  class="btn btn-outline-primary btn-sm"  ' +
                'extend-contract-name="' +
                full.name +
                '"extend-contract="' +
                full.id +
                '"><i class="mdi mdi-update mr-1"></i>Extend Request</button>'
              );

            else if (full.request_type == 'Renewal')
              return (
                '<button  class="btn btn-outline-success btn-sm"  ' +
                'renew-contract-name="' +
                full.name +
                '"renew-contract="' +
                full.id +
                '"><i class="mdi mdi-window-close mr-1"></i>Renew Request</button>'
              );
              ...
              return (
                ...
                '" complete-contract="' +
                full.id +
                '" ><i class="mdi mdi-check"></i>Contract Completed</button>'
              );
            else if (full.request_type == 'Terminate')
              return (
                '<button  class="btn btn-outline-danger btn-sm"  ' +
                'terminate-contract-name="' +
                full.name +
                '"terminate-contract="' +
                full.id +
                '"><i class="mdi mdi-window-close mr-1"></i>Terminate Request</button>'
              );
            else {
              if (full.status == 1)
                return (
            ...
            action: function (e: any, dt: any, node: any, config: any) {
              ...
              for (let i = 0; i < rows.length; i++) {
                ...
                Swal.fire({
                  ...
                  html: '<div id="quill-container">' + rows[i].sla_description + '</div>', // Use html property to render HTML content
                  icon: 'warning',
                  ...
                  preConfirm: () => {
                    ...
                    rows[i].sla_description = quill.root.innerHTML;
                  }
                })
            ...
            action: function (e: any, dt: any, node: any, config: any) {
              ...
              for (let i = 0; i < rows.length; i++) {
                ...
                Swal.fire({
                  ...
                  html: '<div id="quill-container">' + rows[i].payment_term_condition + '</div>', // Use html property to render HTML content
                  icon: 'warning',
                  showCancelButton: true,
                  showConfirmButton: false,
                  confirmButtonColor: '#0acf97',
                  ...
                  preConfirm: () => {
                    // Retrieve Quill content and update this.remark.description
                    rows[i].payment_term_condition = quill.root.innerHTML;
                  }
                })
          ...
          },
          {
            ...
            action: function (e: any, dt: any, node: any, config: any) {
              ...
              for (let i = 0; i < rows.length; i++) {
                that.contract_id = rows[i].id;
                Swal.fire({
                  title: ' Renewal and Termination Options:',
                  html: '<div id="quill-container">' + rows[i].renewal_termination_option + '</div>', // Use html property to render HTML content
                  icon: 'warning',
                  showCancelButton: true,
                  showConfirmButton: false,
                  confirmButtonColor: '#0acf97',
                  ...
                  preConfirm: () => {
                    // Retrieve Quill content and update this.remark.description
                    rows[i].renewal_termination_option = quill.root.innerHTML;
                  }
                })
          ...
          render: function (data: any, type: any, full: any) {
            ...
            if (full.status == 2)
              ...
              return (
                ...
                '" reject-payment="' +
                full.id +
                '"><i class="mdi mdi-window-close"></i>Reject</button>'
              );
            else if (full.status == 1 && full.request_type == null)
              return (
    ...
    this.dtOptions_view_remarks = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.serviceChecker.getContractRemark(this.request_id).subscribe(
          async (resp: any) => {
  ...
  onSubmitReason() {
    if (this.reasonForm.valid) {
      ...
      if (this.reason == "Reject Payment Request") {
        this.RejectContractRequest(this.payment_id, this.remark, this.reason)
  ...
  ContractSubmit() {
    ...
    if (
      ...
      if (this.contractDocument.at(0)! != null)
        formData.append('total_amount', this.contractDocument.at(0)!);
  ...
  paymentSubmit() {
    ...
    if (
      this.paymentForm.valid && this.confirmation == true
    ) {
      ...
      if (this.paymentForm.get('paid_amount')?.value)
        formData.append('paid_amount', this.paymentForm.get('paid_amount')?.value);
      ...
      async (data) => {
        if (data == true) {
          ...
          if (this.contract_id != null) {
            ...
            this.add_payment_modal.hide();
            this.paymentForm.reset();
            if (!this.extend_renew || this.reason !== 'Renewal' || this.reason !== 'Extend')
              this.payment_detail_modal.show();
            ...
            this.utilService.refreshToken().subscribe(
              (data) => {
                if (data === true) {
                  ...
                  this.localStorageService.clear('refresh_token_requested');
                  //================================================================================
                  this.service.registerVendor(formData).subscribe(
                    async (data) => {
                      if (data == true) {
                        this.SuccessSwalUpdateVendor.fire();
                        if (this.contract_id != null) {
  ...
  saveRemark(request_id: any, remark: Remark) {
    ...
    this.serviceChecker.contractRemark(request_id, remark).subscribe(
      async (data) => {
        if (data == true) {
          Swal.fire({
            icon: 'success',
            title: 'Remark',
            text: 'Your remark sent successfuly!',
          });
  ...
  getReason(contract_id: any) {
    var type = 'contract'
    this.serviceChecker.getReason(contract_id, type).subscribe(
      async (data) => {
        if (data != null) {
          this.remark = data;
          console.log(
  ...
  AcceptContractRequest(contract_id: any, remark: Remark) {
    var type = 'contract'
    this.serviceChecker
      .AcceptVendorRequest(contract_id, remark, type)
      .subscribe(
        (data: any) => {
          this.datatableElement.dtInstance.then(
            (dtInstance: DataTables.Api) => {
  ...
  download_view() {
    this.save_as = "Renewal_contract_document"
    const sw = Swal.fire({
    ...
    this.service.downloadContractFilesForRenew(this.contract_id).subscribe((event: any) => {
  ...
  download() {
    ...
    // this.downloadProgress(event,'vendor');
    this.service.downloadContractFiles(this.contract_id).subscribe((event: any) => {
      this.downloadProgress(event);
    });
  }
  clearAllSearches() {
  ...
  ngAfterViewInit(): void {
    ...
    this.datatableElement.dtInstance.then((dtInstance: DataTables.Api) => {
      ...
      console.log('kewch new    kkkkkkkkkkkkk ' + this.searchType)
      if (this.searchType != '') {
        console.log('hhhhhhhhhhhhhhhh    kkkkkkkkkkkkk ' + this.searchType)
        ...
        if (this.searchType.startsWith('vendor_name')) {
          ...
          dtInstance.column(columnTitles.indexOf(this._24th_column_title)).search('^Renew Request$', true, false)
        }
    ...
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('edit-contract')) {
        ...
        Swal.fire({
          ...
          text:
            'Are you sure? you are about to Extend a contract id:' + event.target.getAttribute('extend-contract') + '.',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            ...
            this.serviceChecker.getextend_renewal_for_approved(event.target.getAttribute('extend-contract')).subscribe(
              async (data) => {
                console.log(
                ...
                if (data != null) {
                  ...
                  this.pre_end_date = data[0].end_date;
                  this.pre_total_amount = data[0].total_amount;
                  this.pre_contract_document_document = data[0].contract_document_document;
                  this.renew_contract_document_document = data[0].confirmation_document
                  if (!isNaN(new Date(data[0].payment_end_date).getTime())) {
                    this.maxDate = this.contractRenewalForm.get("end_date")?.patchValue(this.formatDate(data[0].payment_end_date));
                  }


                  if (!isNaN(new Date(data[0].payment_start_date).getTime())) {
                    this.minDate = this.contractRenewalForm.get("start_date")?.patchValue(this.formatDate(data[0].payment_start_date));
                  }

                  // this.end_date = this.formatDate(data.end_date);
                  const num = data[0].paid_amount.toString().replace(/,/g, ''); // Convert to string and remove existing commas
                  const parts = num.split('.'); // Split the number into parts before and after decimal
                  parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ','); // Add commas to the integer part
                    ...
                    this.utilService.refreshToken().subscribe(
                      (data) => {
                        if (data == null) {
                          ...
                          this.localStorageService.clear('refresh_token_requested');
                          //================================================================================
                          this.serviceChecker.getextend_renewal_for_approved(this.contract_id).subscribe(
                            async (data) => {
                              if (data != null) {
                                this.pre_end_date = data[0].end_date;
                                this.pre_total_amount = data[0].total_amount;
            ...
            this.reason = "Extend";
            this.contractRenewalForm.get("total_amount")?.patchValue(this.pre_total_amount);
            this.extend_renew = true;
            this.contract_reneal_modal.show();
        ...
        // this.router.navigateByUrl(
        //   'maker/view-contracts/edit/' + event.target.getAttribute('extend-contract')
        // );
        ...
        Swal.fire({
          ...
          text:
            'Are you sure? you are about to Renew a contract id:' + event.target.getAttribute('renew-contract') + '.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: 'green',
          cancelButtonColor: '#3085d6',
            ...
            this.serviceChecker.getextend_renewal_for_approved(this.contract_id).subscribe(
              async (data) => {
                if (data != null) {
                  ...
                  this.renew_contract_document_document = data[0].confirmation_document
                  if (!isNaN(new Date(data[0].payment_end_date).getTime())) {
                    this.contractRenewalForm.get("end_date")?.patchValue(this.formatDate(data[0].payment_end_date));
                  }


                  if (!isNaN(new Date(data[0].payment_start_date).getTime())) {
                    this.contractRenewalForm.get("start_date")?.patchValue(this.formatDate(data[0].payment_start_date));
                  }
      ...
      } else if (event.target.hasAttribute('approve-contract')) {
        var contract_id = event.target.getAttribute('approve-contract');
        Swal.fire({
          title: 'Approve contract with id : ' + contract_id,
          text:
          ...
          if (result.isConfirmed) {
            this.serviceChecker.ApproveContract(contract_id).subscribe(
              async (data) => {
                if (data == true) {
                  Swal.fire({
                    icon: 'success',
      ...
      } else if (event.target.hasAttribute('reject-contract')) {
        // var vendor_name = event.target.getAttribute('activate-vendor-name');
        var contract_id = event.target.getAttribute('reject-contract');
        Swal.fire({
          title: 'Reject contract! ',
          text:
            'Are you sure? you are about to reject a contract request.',
          icon: 'warning',
          showCancelButton: true,
      ...
      } else if (event.target.hasAttribute('remark-contract')) {
        var selected_request_id = event.target.getAttribute('remark-contract');
      ...
      else if (event.target.hasAttribute('hold-contract')) {
        this.reason = "Hold Contract Request"
        this.contract_id = event.target.getAttribute('hold-contract');
        this.getReason(event.target.getAttribute('hold-contract'));
        Swal.fire({
          title: '<p class="text-warning">Hold Contract:' + "</P>",
          text: ' Are you sure ? you are Acept or Reject  hold request depend on requester reason.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#0acf97',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            Swal.fire({
              title: '<p class="text-primary">Sent from: ' + this.remark.created_by + " on " + this.remark.created_date + ". Hold contract id  " + event.target.getAttribute('hold-contract') + "</p>",
              ...
              if (result.isConfirmed) {
                ...
                this.AcceptContractRequest(event.target.getAttribute('hold-contract'), this.remark);

              } else if (result.dismiss === Swal.DismissReason.close) {
              }
              else {
                this.reason = 'Reject hold request'
                this.reason_modal.show();
              }
            });
      ...
      else if (event.target.hasAttribute('unhold-contract')) {
        this.reason = "Unhold Contract Request"
        this.contract_id = event.target.getAttribute('unhold-contract');
        this.getReason(event.target.getAttribute('unhold-contract'));
        Swal.fire({
          title: '<p class="text-warning">Unhold Contract:' + "</P>",
          text: ' Are you sure ? you are Acept or Reject  Unhold request depend on requester reason.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#0acf97',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            Swal.fire({
              title: '<p class="text-primary">Sent from: ' + this.remark.created_by + " on " + this.remark.created_date + ". Unhold contract id  " + event.target.getAttribute('Unhold-contract') + "</p>",
              html: '<div id="quill-container">' + this.remark.description + '</div>', // Use html property to render HTML content
              icon: 'warning',
              showCancelButton: true,
              showCloseButton: true,
              ...
              if (result.isConfirmed) {
                this.remark.title = this.reason
                this.AcceptContractRequest(event.target.getAttribute('Unhold-contract'), this.remark);

              } else if (result.dismiss === Swal.DismissReason.close) {
              }
              else {
                this.reason = 'Reject unhold request'
                this.reason_modal.show();
              }
            });
      ...
      } else if (event.target.hasAttribute('complete-contract')) {
        this.reason = "Complete Contract Request"
        this.contract_id = event.target.getAttribute('complete-contract');
        this.getReason(event.target.getAttribute('complete-contract'));
        Swal.fire({
          title: '<p class="text-warning">Complete Contract:' + "</P>",
          text: ' Are you sure ? you are Acept or Reject  Complete request depend on requester reason.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#0acf97',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            Swal.fire({
              title: '<p class="text-primary">Sent from: ' + this.remark.created_by + " on " + this.remark.created_date + ". Terminate contract id  " + event.target.getAttribute('complete-contract') + "</p>",
              html: '<div id="quill-container">' + this.remark.description + '</div>', // Use html property to render HTML content
              icon: 'warning',
              showCancelButton: true,
              showCloseButton: true,
              ...
              if (result.isConfirmed) {
                this.remark.title = this.reason
                this.AcceptContractRequest(event.target.getAttribute('complete-contract'), this.remark);

              } else if (result.dismiss === Swal.DismissReason.close) {
              }
              else {
                this.reason = 'Reject Complete request'
                this.reason_modal.show();
              }
            });
        ...
        this.reason = "Terminate Contract Request"
        this.contract_id = event.target.getAttribute('terminate-contract');
        this.getReason(event.target.getAttribute('terminate-contract'));
        Swal.fire({
          title: '<p class="text-warning">Terminate Contract:' + "</P>",
          text: ' Are you sure ? you are Acept or Reject  Terminate request depend on requester reason.',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#0acf97',
          cancelButtonColor: '#3085d6',
          ...
          if (result.isConfirmed) {
            Swal.fire({
              title: '<p class="text-primary">Sent from: ' + this.remark.created_by + " on " + this.remark.created_date + ". Terminate contract id  " + event.target.getAttribute('terminate-contract') + "</p>",
              html: '<div id="quill-container">' + this.remark.description + '</div>', // Use html property to render HTML content
              icon: 'warning',
              showCancelButton: true,
              showCloseButton: true,
              ...
              if (result.isConfirmed) {
                this.remark.title = this.reason
                this.AcceptContractRequest(event.target.getAttribute('terminate-contract'), this.remark);

              } else if (result.dismiss === Swal.DismissReason.close) {
              }
              else {
                this.reason = 'Reject terminate request'
                this.reason_modal.show();
              }
            });
      ...
      } else if (event.target.hasAttribute('approve-payment')) {
        var payment_id = event.target.getAttribute('approve-payment');
        Swal.fire({
          title: 'Approve Payment Id: ' + payment_id,
          text:
            'Are you sure? you are about to Approve payment request Payment Id:  ' +
            payment_id + '',
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: 'green',
          ...
          if (result.isConfirmed) {
            ...
            this.payment_id = event.target.getAttribute('approve-payment');
            this.reason = "approve";
            this.IdPayment = event.target.getAttribute('approve-payment');
            this.serviceChecker.approvePayment(event.target.getAttribute('approve-payment')).subscribe(
              async (data) => {
                if (data == true) {
                  $('#view_detail').DataTable().ajax.reload()
                  Swal.fire({
                    icon: 'success',
                    title: 'Success...',
                    text: 'Payment Id :' + event.target.getAttribute('approve-payment') + ' Successfully approved!',
                  });
                    ...
                    this.utilService.refreshToken().subscribe(
                      (data) => {
                        if (data === true) {
                          ...
                          this.localStorageService.clear('refresh_token_requested');
                          //================================================================================
                          this.serviceChecker.approvePayment(event.target.getAttribute('pending-payment')).subscribe(
                            async (data) => {
                              if (data == true) {
                                $('#view_detail').DataTable().ajax.reload()
                                Swal.fire({
      ...
      } else if (event.target.hasAttribute('reject-payment')) {
        var payment_id = event.target.getAttribute('reject-payment');
        Swal.fire({
          title: 'Reject Payment Id: ' + payment_id,
          text:
            'Are you sure? you are about to Reject payment request:  ' +
            '',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            ...
            this.payment_id = event.target.getAttribute('reject-payment');
            this.reason = "Reject Payment Request";
            // this.IdPayment=event.target.getAttribute('reject-payment');
            this.reason_modal.show();
      ...
      //       this.reason = "Extend";
      //       this.contractdocument == true;
      //       this.contractRenewalForm.get("end_date")?.patchValue(this.formatDate(this.end_date));
